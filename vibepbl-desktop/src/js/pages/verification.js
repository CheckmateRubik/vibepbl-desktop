...
function row(hypothesis, index, problemId) { const state = hypothesis.validation || (hypothesis.status === 'green' ? 'correct' : hypothesis.status === 'yellow' ? 'yellow' : 'pending'); const type = state === 'correct' ? 'confirmed' : state === 'yellow' ? 'investigating' : state; const label = state === 'correct' ? 'Confirmed ✓' : state === 'wrong' ? 'Wrong ✗' : state === 'yellow' ? 'Investigating ⚡' : 'Unchecked'; return `<div class="hypothesis-row ${type !== 'pending' ? `status-${type}` : ''}"><button class="status-badge ${type !== 'pending' ? `status-${type}` : ''}" data-cycle-status="${problemId}|${hypothesis.id}">${label}</button><span class="code-badge">H${index + 1}</span><span class="hypothesis-text">${esc(hypothesis.text)}</span><button class="button button-ghost" data-edit-hypothesis="${problemId}|${hypothesis.id}">✎</button><button class="button button-ghost" data-delete-hypothesis="${problemId}|${hypothesis.id}">🗑</button></div>`; }
