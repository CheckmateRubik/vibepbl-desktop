...
function hypothesisRow(hypothesis, index, locked) { const type = hypothesis.validation === 'wrong' ? 'wrong' : hypothesis.status === 'green' ? 'confirmed' : hypothesis.status === 'yellow' ? 'investigating' : ''; const label = type === 'confirmed' ? 'Confirmed ✓' : type === 'investigating' ? 'Investigating ⚡' : type === 'wrong' ? 'Wrong ✗' : 'Unchecked'; return `<article class="hypothesis-row ${type ? `status-${type}` : ''}"><button class="status-badge ${type ? `status-${type}` : ''}" data-cycle="${hypothesis.id}" ${locked ? 'disabled' : ''}>${label}</button><span class="code-badge">H${index + 1}</span><span class="hypothesis-text">${esc(hypothesis.text)}</span><div class="item-actions"><button class="button button-ghost" data-edit-hyp="${hypothesis.id}" ${locked ? 'disabled' : ''}>✎</button><button class="button button-ghost" data-delete-hyp="${hypothesis.id}" ${locked ? 'disabled' : ''}>🗑</button></div></article>`; }
