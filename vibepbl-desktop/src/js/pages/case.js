...
export function renderCase(ctx) {
  ...
  document.getElementById('page').innerHTML = `
    ...
    <section class="card">
      ...
      <div id="case-editor" class="rich-editor" contenteditable="${!locked}" data-placeholder="Enter the case scenario…">${session.caseText || ''}</div>
    </section>
    <section class="card">
  ...
  editor?.addEventListener('input', () => ctx.setField('caseText', editor.innerHTML, false));
...
function openAnnotation(ctx, imageId) {
  ...
  const pins = () => image.pins.map((pin, index) => `<button class="pin" data-pin="${pin.id}" style="left:${pin.x}%;top:${pin.y}%" title="${esc(pin.label)}">${index + 1}</button>`).join('');
  const index = () => image.pins.map((pin, number) => `<div class="list-item"><span class="code-badge">${number + 1}</span><div class="list-item-main">${esc(pin.label)}<div class="small muted">${pin.x}%, ${pin.y}%</div></div>${ctx.session.isAct1Completed ? '' : `<button class="button button-ghost" data-remove-pin="${pin.id}">🗑</button>`}</div>`).join('');
  openModal(esc(image.originalName), `<div class="annotation-stage"><div class="annotation-image-wrap"><img id="annotation-image" src="${ctx.API.imageUrl(image.localPath)}" alt="${esc(image.originalName)}"><div id="pin-layer">${pins()}</div></div></div><p class="small muted">${ctx.session.isAct1Completed ? 'Act 1 is locked.' : 'Click the image to add a numbered clinical finding.'}</p><div id="pin-index" class="pin-index">${index()}</div>`, root => {
