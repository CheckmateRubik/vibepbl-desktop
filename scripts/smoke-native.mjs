...
try {
  ...
  await delay(800);
  const shellState = await evaluate(client, `(async () => { await document.fonts.ready; const logo=document.querySelector('.brand-logo'); const text=document.body.innerText; const css=[...document.styleSheets].flatMap(sheet => { try { return [...sheet.cssRules].map(rule => rule.cssText); } catch { return []; } }).join(' '); return { fontFamily:getComputedStyle(document.body).fontFamily, thaiUiLoaded:document.fonts.check('16px "VibePBL Retro"','ภาษาไทย'), hasThaiUnicodeRange:css.toUpperCase().includes('U+E00-E7F'), logoSrc:logo?.getAttribute('src'), logoLoaded:Boolean(logo?.complete && logo.naturalWidth > 0), removedWorkspaceCopy:!text.includes('Private workspace') && !text.includes('Saved locally on this device'), removedJsonUi:!text.includes('.pbl.json'), savedStatusRemoved:!document.querySelector('.save-indicator,#save-indicator'), groupMembersRemoved:!text.includes('Group members') && !document.getElementById('member-name'), currentSection:document.getElementById('current-section')?.textContent, hasCustomScrollbar:css.includes('::-webkit-scrollbar-thumb') && css.includes('::-webkit-scrollbar-button') }; })()`);
  if (!shellState.fontFamily.includes('VibePBL Retro') || !shellState.thaiUiLoaded || !shellState.hasThaiUnicodeRange || shellState.logoSrc !== './assets/app-icon.svg' || !shellState.logoLoaded || !shellState.removedWorkspaceCopy || !shellState.removedJsonUi || !shellState.savedStatusRemoved || !shellState.groupMembersRemoved || shellState.currentSection !== 'Session & settings' || !shellState.hasCustomScrollbar) throw new Error(`App shell simplification failed: ${JSON.stringify(shellState)}`);
