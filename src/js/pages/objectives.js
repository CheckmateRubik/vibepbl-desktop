...
export function renderObjectives(ctx) {
  ...
  document.getElementById('page').innerHTML = `
    ${pageHeader('Learning objectives', 'Group related problems under learning objectives. A problem can support multiple objectives.', `<button id="print-act1" class="button button-secondary">Print Act 1</button><button id="toggle-lock" class="button ${locked ? 'button-secondary' : 'button-success'}">${locked ? 'Unlock Act 1' : '✓ Mark Act 1 complete'}</button>`)}
  ...
  document.querySelectorAll('[data-edit]').forEach(button => button.addEventListener('click', () => {
    ...
    openFormModal('Edit learning objective', [{ name:'text', label:'Learning objective', type:'textarea', value:objective.text }], ({ text }) => {
...
function objectiveCard(ctx, objective, index, locked) {
  ...
  const available = ctx.session.problems.filter(problem => !objective.linkedProblemIds.includes(problem.id));
  return `<article class="objective-card"><div class="objective-head"><span class="code-badge">LO${index + 1}</span><div class="objective-text">${esc(objective.text)}</div><button class="button button-ghost" data-edit="${esc(objective.id)}" ${locked ? 'disabled' : ''}>✎</button><button class="button button-ghost" data-delete="${esc(objective.id)}" ${locked ? 'disabled' : ''}>🗑</button></div>
    <div class="objective-compact-links"><div class="linked-problem-list" aria-label="Linked problems">${linked || '<span class="muted small">No linked problems</span>'}</div><label class="objective-link-picker"><span class="small muted">Add problem</span><select class="select" data-link-select="${esc(objective.id)}" aria-label="Add a problem to LO${index + 1}" ${locked || !available.length ? 'disabled' : ''}><option value="">${available.length ? 'Select a problem…' : 'All problems linked here'}</option>${available.map(problem => { const problemIndex = ctx.session.problems.indexOf(problem); return `<option value="${esc(problem.id)}">P${problemIndex + 1} · ${esc(problem.text)}</option>`; }).join('')}</select></label></div>
  </article>`;
}
...
export function normalizeLinks(objectives, problems) {
