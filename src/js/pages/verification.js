...
export function renderVerification(ctx) {
  ...
  document.querySelectorAll('[data-edit-hypothesis]').forEach(button => button.addEventListener('click', () => { const [problemId, hypothesisId] = button.dataset.editHypothesis.split('|'); const hypothesis = ctx.session.problems.find(item => item.id === problemId).hypotheses.find(item => item.id === hypothesisId); openFormModal('Revise hypothesis', [{ name:'text', label:'Hypothesis', type:'textarea', value:hypothesis.text }], ({ text }) => { hypothesis.text = text.trim(); ctx.setField('problems', ctx.session.problems); ctx.render(); }); }));
